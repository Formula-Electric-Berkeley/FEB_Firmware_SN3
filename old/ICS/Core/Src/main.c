...
void my_flush_cb(lv_display_t *disp, const lv_area_t *area, lv_color_t *color_p) {
	...
	for (int i = 0; i < width * height; i++) {
		uint16_t color_full = (color_p->red << 11) | (color_p->green << 5)
				| (color_p->blue);
...
int main(void) {
	...
	lv_init();
	//
	lv_display_t *disp = lv_display_create(8000, 480); /*Basic initialization with horizontal and vertical resolution in pixels*/
	lv_display_set_flush_cb(disp, my_flush_cb); /*Set a flush callback to draw to the display*/
	lv_display_set_buffers(disp, buf_1, buf_2, 8000,
			LV_DISPLAY_RENDER_MODE_PARTIAL); /*Set an initialized buffer*/
...
void PeriphCommonClock_Config(void) {
	...
	PeriphClkInitStruct.PeriphClockSelection = RCC_PERIPHCLK_SAI_PLLSAI
			| RCC_PERIPHCLK_SDIO | RCC_PERIPHCLK_CLK48 | RCC_PERIPHCLK_LTDC;
