...
void FEB_Validate_Readings_Validate_Voltages(uint8_t ic, float expected_voltage, uint8_t curr_cell) {
	for (uint8_t cell = 0; cell < CELLS_TO_TEST; cell++) {
		...
		if (curr_cell == cell) {
			if (polled_voltage > expected_voltage + CELL_VOLTAGE_THRESHOLD_mV * 0.0001 || polled_voltage < expected_voltage - CELL_VOLTAGE_THRESHOLD_mV * 0.0001) {
				char errmsg[128];
			...
			if (polled_voltage > CELL_DEFAULT_VOLTAGE + CELL_VOLTAGE_THRESHOLD_mV * 0.0001 || polled_voltage < CELL_DEFAULT_VOLTAGE - CELL_VOLTAGE_THRESHOLD_mV * 0.0001) {
				char errmsg[128];
